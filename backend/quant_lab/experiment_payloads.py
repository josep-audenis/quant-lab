...
from .json_codec import experiment_from_json
...
def build_draft_experiment(payload: dict[str, Any], existing: Experiment | None = None) -> Experiment:
    try:
        now = datetime.now(UTC)
        strategy_kind = StrategyKind(required_str(payload, "strategy_kind"))
        universe = tuple(split_symbols(payload.get("universe")))
        parameters = strategy_parameters(strategy_kind, payload)
        program = strategy_program(strategy_kind, universe, parameters)
        ...
        return Experiment(
            ...
            strategy=StrategyConfig(
                ...
                rules=tuple(rules(strategy_kind, universe, parameters, payload)),
            ),
            backtest=BacktestConfig(
                start_date=required_date(payload, "start_date"),
                end_date=required_date(payload, "end_date"),
                initial_capital=required_float(payload, "initial_capital"),
                benchmark=required_str(payload, "benchmark"),
                ...
                cost_model=CostModel(
                    commission_bps=coerce_float(payload.get("commission_bps", 0.0)),
                    slippage_bps=coerce_float(payload.get("slippage_bps", 0.0)),
                    min_commission=coerce_float(payload.get("min_commission", 0.0)),
                ),
                cash_policy=CashPolicy(payload.get("cash_policy", CashPolicy.HOLD_CASH.value)),
                risk_free_rate=coerce_float(payload.get("risk_free_rate", 0.0)),
            ...
            hypothesis=required_str(payload, "hypothesis"),
            notes=optional_str(payload.get("notes")),
        )
...
def experiment_blueprints() -> list[dict[str, Any]]:
    return [
        {
            "id": "buy_and_hold",
            "name": "Core ETF hold",
...
def parse_experiment_payload(payload: dict[str, Any]) -> Experiment:
    try:
        return experiment_from_json(normalize_payload(payload))
    except DomainError as exc:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail=str(exc)) from exc
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail=str(exc)) from exc
...
def required_date(payload: dict[str, Any], key: str) -> date:
    return date.fromisoformat(required_str(payload, key))
...
def strategy_parameters(strategy_kind: StrategyKind, payload: dict[str, Any]) -> dict[str, Any]:
    if strategy_kind is StrategyKind.BUY_AND_HOLD:
        return {}
    if strategy_kind is StrategyKind.MOVING_AVERAGE_FILTER:
        return {
            "window": coerce_int(payload.get("ma_window", 200), "ma_window"),
    ...
    if strategy_kind is StrategyKind.MOMENTUM_ROTATION:
        return {
            "lookback_months": coerce_int(payload.get("lookback_months", 12), "lookback_months"),
            "top_n": coerce_int(payload.get("top_n", 1), "top_n"),
...
def ensure_strategy_program(experiment: Experiment) -> Experiment:
...
def rules(
    ...
    customrules = payload.get("rules")
    if isinstance(customrules, str) and customrules.strip():
        return [line.strip() for line in customrules.splitlines() if line.strip()]
    if strategy_kind is StrategyKind.BUY_AND_HOLD:
