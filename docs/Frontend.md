...
- `src/App.tsx` wraps `LabsScreen` in `AppShell`.
...
- `RuleBuilder.tsx`, `RuleWorkbench.tsx`, `components/workbench/` - strategy/rule editing and sweeps.
- `ResultsPanel.tsx`, `components/results/` - metrics, charts, diagnostics, trade tables.

## Styles

- `src/styles.css` imports/holds global styles.
- `src/styles/` splits base, forms, builder, experiments, results, tables/sweeps.
